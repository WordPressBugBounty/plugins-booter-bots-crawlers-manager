...
class RobotsWriter {
	...
	function add_cron_schedule( $schedules ) {
		if ( ! isset( $schedules['weekly'] ) ) {
			$schedules['weekly'] = [
				...
				'display' => __( 'Once Weekly', 'booter-bots-crawlers-manager' )
			];
		}
    ...
    public static function is_robots_generated_by_booter() {
        if ( ! self::$is_robots_generated_by_booter ) {
            $content = file_get_contents( self::get_robots_path() );

            self::$is_robots_generated_by_booter = ( $content && 1 === preg_match( '/Robots\.txt generated by Booter - (Crawlers|Crwalers) Manager/i', $content ) );
        }
    ...
    function maybe_write_robots_file( $settings = null ) {
        ...
        if ( ! WP_Filesystem() ) {
            error_log( 'Booter: Failed to initialize WP_Filesystem in RobotsWriter.' );
            return;
        }

        $robots_path = self::get_robots_path();
        if ( self::robots_file_exists() && ! self::is_robots_generated_by_booter() ) {
            $wp_filesystem->move( $robots_path, $robots_path . '.' . time() . '.old' );
        }

        $date = gmdate( 'r' );

        $content = "# Robots.txt generated by Booter - Crawlers Manager at " . $date . "\n" .
        ...
        if ( Utilities::bool_value( $settings['robots']['block_all'] ) ) {
            ...
            $wp_filesystem->put_contents( $robots_path, $content, FS_CHMOD_FILE );
            return;
        }
        ...
        if ( isset( $settings['robots']['manage_type'] ) && 'simple' === $settings['robots']['manage_type'] ) {
            ...
            foreach ( $settings['robots']['useragents'] as $ua ) {
                ...
                if ( ! empty( $ua['disallow'] ) ) {
                    $ua['disallow'] = is_array( $ua['disallow'] ) ? $ua['disallow'] : json_decode( $ua['disallow'], true );
                    $ua['disallow'] = is_array( $ua['disallow'] ) ? $ua['disallow'] : [];
                ...
                if ( ! empty( $ua['allow'] ) ) {
                    $ua['allow'] = is_array( $ua['allow'] ) ? $ua['allow'] : json_decode( $ua['allow'], true );
                    $ua['allow'] = is_array( $ua['allow'] ) ? $ua['allow'] : [];
