...
class Settings {
	...
	public function admin_bar_menu( $wp_admin_bar ) {
		...
		$on = sprintf( '<span style="color: #48BB78;">%s</span>', __( 'Enabled', 'booter-bots-crawlers-manager' ) );
		$off = sprintf( '<span style="color: #F56565;">%s</span>', __( 'Disabled', 'booter-bots-crawlers-manager' ) );

		$items = [
			'bad_robots' => [ __( 'Block Bad Robots', 'booter-bots-crawlers-manager' ), Utilities::bool_value( $settings['block']['block_bad_robots'] ) ],
			'robots_txt' => [ __( 'robots.txt Management', 'booter-bots-crawlers-manager' ), Utilities::bool_value( $settings['robots']['enabled'] ) ],
			'reject'     => [ __( 'Reject Links', 'booter-bots-crawlers-manager' ), Utilities::bool_value( $settings['block']['enabled'] ) ],
			'rate_limit' => [ __( 'Rate Limiting', 'booter-bots-crawlers-manager' ), Utilities::bool_value( $settings['rate_limit']['enabled'] ) ],
			'log_404'    => [ __( '404 Logging', 'booter-bots-crawlers-manager' ), Utilities::bool_value( $settings['log_404']['enabled'] ) ],
		];
		...
		$wp_admin_bar->add_menu( [
			'id'        => 'booter',
			'title'     => $svg . __( 'Booter - Bots & Crawlers Manager', 'booter-bots-crawlers-manager' ) . '&nbsp;&nbsp;' . implode( '&nbsp;', $main_title_icons ),
	...
	public function enqueue_scripts() {
		...
		wp_localize_script( 'booter-notices', 'wp_booter_notices', [
			...
			'confirm_disable_plugins' => __( 'Are you sure you want to disable the plugins?', 'booter-bots-crawlers-manager' ),
		]);
		...
		wp_localize_script( 'booter-options', 'wp_booter', [
			...
			'trans' => [
				'toggle_panel' => __( 'Toggle Panel', 'booter-bots-crawlers-manager' ),
				'confirm_title' => __( 'Are You Sure?', 'booter-bots-crawlers-manager' ),
				'delete' => __( 'Delete', 'booter-bots-crawlers-manager' ),
				'cancel' => __( 'Cancel', 'booter-bots-crawlers-manager' ),
				'allow' => __( 'Allow', 'booter-bots-crawlers-manager' ),
				'disallow' => __( 'Disallow', 'booter-bots-crawlers-manager' ),
				'add' => __( 'Add', 'booter-bots-crawlers-manager' ),
				'remove' => __( 'Remove', 'booter-bots-crawlers-manager' ),
				'no' => __( 'No', 'booter-bots-crawlers-manager' ),
				'yes' => __( 'Yes', 'booter-bots-crawlers-manager' ),
				'disabled' => __( 'Disabled', 'booter-bots-crawlers-manager' ),
				'enabled' => __( 'Enabled', 'booter-bots-crawlers-manager' ),
				'manage' => __( 'Manage', 'booter-bots-crawlers-manager' ),
				'save' => __( 'Save', 'booter-bots-crawlers-manager' ),
				'add_string_to_block' => __( 'Add a string to block', 'booter-bots-crawlers-manager' ),
				'add_allowed_url' => __( 'Add allowed URL', 'booter-bots-crawlers-manager' ),
				'add_disallowed_url' => __( 'Add disallowed URL', 'booter-bots-crawlers-manager' ),
				'all_crawlers' => __( 'All Crawlers', 'booter-bots-crawlers-manager' ),
				'add_new_crawler' => __( 'Add New Crawler', 'booter-bots-crawlers-manager' ),
				'user_agent' => __( 'User-Agent', 'booter-bots-crawlers-manager' ),
				'crawl_rate' => __( 'Crawl Rate', 'booter-bots-crawlers-manager' ),
				'not_defined' => __( 'Not Defined', 'booter-bots-crawlers-manager' ),
                /* translators: %s: number of seconds */
				'n_seconds' => __( '%s Seconds', 'booter-bots-crawlers-manager' ),
                /* translators: %s: number of minutes */
				'n_minutes' => __( '%s Minutes', 'booter-bots-crawlers-manager' ),
				'between_scans' => __( 'Delay between scans', 'booter-bots-crawlers-manager' ),
				'crawl_rate_description' => __( 'Use this rule to throttle a crawler if they are crawling your website too frequently. Some crawlers (eg. GoogleBot) will ignore this rule, and some others might interpret it a little differently.', 'booter-bots-crawlers-manager' ),
				'no_crawl_rate_description' => __( 'No defined rate will allow the robots to crawl at their own discretion.', 'booter-bots-crawlers-manager' ),
                /* translators: %s: crawler name */
				'x_ignores_rule' => __( '%s ignores this rule', 'booter-bots-crawlers-manager' ),
				'robots_include_block_strings' => __( 'Disallow Rejected Strings', 'booter-bots-crawlers-manager' ),
				'robots_include_block_strings_description' => __( 'Include the rejected strings in the disallowed links list.', 'booter-bots-crawlers-manager' ),
				'robots_include_block_strings_link' => __( 'Manage Rejected Strings', 'booter-bots-crawlers-manager' ),
				'disallow_dashboard' => __( 'Disallow Dashboard', 'booter-bots-crawlers-manager' ),
                /* translators: %s: dashboard URL */
				'disallow_dashboard_description' => sprintf( __( 'Block all URLs starting with %s.', 'booter-bots-crawlers-manager' ), sprintf( '<code>%s</code>', admin_url() ) ),
				'allow_ajax' => __( 'Allow AJAX', 'booter-bots-crawlers-manager' ),
                /* translators: %s: admin-ajax.php URL */
				'allow_ajax_description' => sprintf( __( 'Blocking the dashboard will block the %s file as well, if your theme uses AJAX to display content - AJAX has to be enabled for search engines to be able to see this content.', 'booter-bots-crawlers-manager' ), sprintf( '<code>%s</code>', admin_url( 'admin-ajax.php' ) ) ),
				'new_crawler_useragent' => _x( 'New Crawler', 'tab label for new crawler when no user agent is defined', 'booter-bots-crawlers-manager' ),
				'user_agent_star' => __( 'Use * as a user agent to affect all crawlers.', 'booter-bots-crawlers-manager' ),
				'no_crawlers_defined' => __( 'You currently do not have any crawler settings defined.', 'booter-bots-crawlers-manager' ),
				'user_agent_exists' => __( 'This user-agent already exists.', 'booter-bots-crawlers-manager' ),
				'user_agent_invalid' => __( 'The user-agent can not contain spaces, tabs, and any of the following charactes: ()<>@,;:"/[]?={}', 'booter-bots-crawlers-manager' ),
                /* translators: %s: crawler name */
				'confirm_delete_crawler' => __( 'Are you sure you want to delete the crawler "%s"?', 'booter-bots-crawlers-manager' ),
				'disallowed_links' => __( 'Disallowed Links', 'booter-bots-crawlers-manager' ),
				'allowed_links' => __( 'Allowed Links', 'booter-bots-crawlers-manager' ),
				'manage_blocked_bots' => __( 'Manage Blocked Bots', 'booter-bots-crawlers-manager' ),
				'empty_robots_disallow' => __( 'No disallowed links means that all pages will be indexed.', 'booter-bots-crawlers-manager' ),
				'update_bad_robots_list' => __( 'Update Bad Bots From Predefined List', 'booter-bots-crawlers-manager' ),
				'bad_bots_description' => __( 'Any bots included in this list will be automatically blocked. You can update the list from our predefined list of known bad bots by clicking the above link (your current list will not be overwritten).', 'booter-bots-crawlers-manager' ),
                'add_user_agent' => __( 'Add a user agent to block', 'booter-bots-crawlers-manager' ),
				'allow_disallow_wildcards' => __( 'Some crawlers support a limited form of "wildcards" for path values:', 'booter-bots-crawlers-manager' ),
				'allow_disallow_wildcards_star' => __( '<code>*</code> designates 0 or more instances of any valid character.', 'booter-bots-crawlers-manager' ),
				'allow_disallow_wildcards_dollar' => __( '<code>$</code> designates the end of the URL.', 'booter-bots-crawlers-manager' ),
			]
		] );
	...
	public function plugin_action_links( $links ) {
		$links[] = sprintf(
            ...
            __( 'Settings', 'booter-bots-crawlers-manager' )
		);
		return $links;
	}
	...
	public function register_menu() {
		add_options_page(
			__( 'Booter - Bots & Crawlers Manager', 'booter-bots-crawlers-manager' ),
			__( 'Booter - Bots & Crawlers Manager', 'booter-bots-crawlers-manager' ),
			'manage_options',
			'booter',
			[ $this, 'settings_page' ]
	...
	function maybe_notice_404_plugins() {
		...
		<div class="js-booter-404-notice notice notice-warning is-dismissible">
            <p>
                ...
                echo wp_kses_post(
                        sprintf(
                        /* translators: 1: plugin name, 2: count of plugins */
                                _n(
			...
			<p>
				<?php esc_html_e( 'Such redirects prevents Booter from detecting 404 errors as well as being the wrong way to handle broken links.', 'booter-bots-crawlers-manager' ); ?>
				<?php esc_html_e( 'A 404 error is the correct response to invalid URLs while a redirect (30X) tells search engines that the URLs exists (in another location).', 'booter-bots-crawlers-manager' ); ?>
				<br>
				<?php esc_html_e( 'We therefore recommend that you disable these plugins.', 'booter-bots-crawlers-manager' ); ?>
			</p>
			<p>
                <button type="button" class="button button-small js-booter-disable-plugins" data-slugs="<?php echo esc_attr( implode( ',', $plugin_slugs ) ); ?>">
					<?php echo esc_html( _n( 'Disable The Plugin', 'Disable The Plugins', count( $plugin_names ), 'booter-bots-crawlers-manager' ) ); ?>
				</button>
			</p>
		</div>
		<?php
	...
	function maybe_notice_plugins_deactivated() {
		...
		echo '<div class="notice notice-success is-dismissible"><p><strong>' . esc_html__( 'The plugins have been disabled.', 'booter-bots-crawlers-manager' ) . '</strong></p></div>';
    }
